...
const getHoursPerDay = (rawDay) => {
    ...
    if ((dayStart && !lunchStart) || (lunchEnd && !dayEnd)) {
        ...
        if (today.day !== dayStart.day || today.hour > 19) {
            log4js_1.logger.error(dayStart.toISODate());
...
const getHoursPerWeek = (id, week, year) => __awaiter(void 0, void 0, void 0, function* () {
    const workedWeek = yield mysql_1.default.instance.query('days', 'day_start, lunch_start, lunch_end, day_end, extraPause_start, extraPause_end', `employee_id = ${id} and week = ${week} AND year(day_date) = ${year}`);
...
const getDateDetails = (date, id) => __awaiter(void 0, void 0, void 0, function* () {
    const dbResponse = yield mysql_1.default.instance.query('days', 'day_date, day_start, lunch_start, lunch_end, day_end, extraPause_start, extraPause_end, week, employees.fullname', `day_date='${date}' and days.employee_id=${id}`, { employees: 'employee_id' });
...
const getEmployeeSummary = (id, weekNumber, year) => __awaiter(void 0, void 0, void 0, function* () {
    const rawHours = yield getHoursPerWeek(parseInt(id), weekNumber, year);
    const weekHours = rawHours.toFormat('hh:mm');
    const [{ fullname, hourly_pay: hourlyPay }] = yield mysql_1.default.instance.query('employees', 'fullname, hourly_pay', `employee_id = ${id}`); // or fetch /employees
    const weekPay = Math.ceil(rawHours.as('hours') * hourlyPay);
    const weekDates = yield mysql_1.default.instance.query('days', 'day_date', `week = ${weekNumber} and employee_id = ${id} and year(day_date) = ${year}`);
    const detailedDays = yield Promise.all(weekDates.map((day) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        return yield getDateDetails(formatedDate, id.toString());
    })));
    return { weekHours, weekPay, fullname, rawHours, detailedDays, weekNumber };
});
const getWeekTotalSummary = (weekDate) => __awaiter(void 0, void 0, void 0, function* () {
    let weekNumber;
    let yearNumber;
    if (!weekDate) {
        weekNumber = luxon_1.DateTime.local().weekNumber;
        yearNumber = luxon_1.DateTime.local().year;
    }
    else {
        weekNumber = luxon_1.DateTime.fromJSDate(new Date(weekDate)).weekNumber;
        yearNumber = luxon_1.DateTime.fromJSDate(new Date(weekDate)).year;
    }
    const employeesId = yield mysql_1.default.instance.query('employees', 'employee_id', 'active = 1'); // or fetch /employees
    const detailsPerEmployee = yield Promise.all(employeesId.map((employee) => __awaiter(void 0, void 0, void 0, function* () {
        const employeeHours = yield getEmployeeSummary(employee.employee_id.toString(), weekNumber, yearNumber);
        return employeeHours;
    })));
    const total = detailsPerEmployee.reduce((a, b) => {
        return { rawHours: a.rawHours.plus(b.rawHours), weekPay: a.weekPay + b.weekPay };
    }, { rawHours: luxon_1.Duration.fromMillis(0), weekPay: 0 });
...
const getLargePeriodSummary = (query) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const employeesData = yield mysql_1.default.instance.list('employees', 'employee_id, fullname, hourly_pay'); // or fetch /employees
    const workedDaysPerEmployee = yield Promise.all(employeesData.map((employee) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        const days = yield mysql_1.default.instance.query('days', 'day_date', where);
    ...
    const detailedPeriodPerEmployee = yield Promise.all(workedDaysPerEmployee.map((employee) => __awaiter(void 0, void 0, void 0, function* () {
        const dayDetail = yield Promise.all(employee.days.map((day) => __awaiter(void 0, void 0, void 0, function* () {
            const formatedDate = luxon_1.DateTime.fromJSDate(day.day_date).toSQLDate();
            const details = yield getDateDetails(formatedDate, employee.employee_id.toString());
            return details;
        })));
    ...
    const periodNumber = query.month ? `${luxon_1.DateTime.fromJSDate(new Date(query.date)).month}/${selectedYear}` : selectedYear.toString();
...
const getByQuery = (params) => __awaiter(void 0, void 0, void 0, function* () {
    const query = querystring_1.default.parse(params);
    ...
    if (query.employee_id)
        return getEmployeeSummary(query.employee_id, luxon_1.DateTime.local().weekNumber, luxon_1.DateTime.local().year);
    if (query.week)
...
const markSchedules = (body) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const startedDay = yield mysql_1.default.instance.query('days', '*', `employee_id = ${body.id} AND day_date = '${today.toSQLDate()}'`);
    if (startedDay.length === 0) {
        ...
        const newData = yield mysql_1.default.instance.upsert('days', newDay);
    ...
    const newData = yield mysql_1.default.instance.upsert('days', dayUpdate);
...
const updateDate = (body) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const dayExist = yield mysql_1.default.instance.query('days', 'id', `day_date = '${luxon_1.DateTime.fromJSDate(new Date(body.date))}' and employee_id = ${body.employee_id}`);
    ...
    const updatedData = yield mysql_1.default.instance.upsert('days', dayUpdate);
...
const deleteDate = (params) => __awaiter(void 0, void 0, void 0, function* () {
    const query = querystring_1.default.parse(params);
    const dayExist = yield mysql_1.default.instance.query('days', 'id', `day_date = '${query.date}' and employee_id = ${query.employee_id}`);
    if (dayExist.length === 0)
        throw Error('No se encontro esa fecha.');
    const removedDay = yield mysql_1.default.instance.remove('days', `id = ${dayExist[0].id}`);
    if (!removedDay)
